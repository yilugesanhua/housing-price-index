...
test('mini program snapshot covers 70 cities and 120 months', async () => {
  ...
  assert.match(snapshot.datasetAsOf, /^20\d{2}-(0[1-9]|1[0-2])$/)
  assert.equal(snapshot.datasetAsOf, publishedManifest.dataset_as_of)
  assert.match(snapshot.datasetVersion, new RegExp(`^${snapshot.datasetAsOf}-[a-f0-9]{12}$`))
