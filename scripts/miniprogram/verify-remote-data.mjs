...
import { byteLength, sha256, verifyReleaseAgainstSnapshot } from './remote-data-lib.mjs'
...
const errors = verifyReleaseAgainstSnapshot(snapshot, { bootstrap, bootstrapText, manifest, manifestText, current, currentText, cities, totalBytes })
if (errors.length) {
  console.error('Remote release verification failed:')
  for (const error of errors) console.error(`- ${error}`)
  process.exitCode = 1
} else {
  console.log(`Verified ${manifest.dataset_version}: 70 city shards, ${totalBytes} bytes, exact snapshot reconstruction passed`)
}
