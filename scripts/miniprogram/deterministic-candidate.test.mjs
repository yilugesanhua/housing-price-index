...
import { assertCandidatePaths, buildCandidateManifest, compareDevtools, createDeterministicZip, inventoryDigest, normalizeArchiveEntries, parseSnapshot, readZipInventory } from './deterministic-candidate.mjs'
...
test('reads metadata from the approved compressed snapshot export without evaluating it', () => {
  const rawText = "// Generated by npm run miniprogram:data. Do not edit.\nmodule.exports=require('../utils/snapshot-codec.js').decodeSnapshot({\"datasetAsOf\":\"2026-06\",\"datasetVersion\":\"2026-06-example\",\"encodedSeries\":{}})\n"
  const snapshot = parseSnapshot(rawText)
  assert.equal(snapshot.datasetAsOf, '2026-06')
  assert.equal(snapshot.datasetVersion, '2026-06-example')
  assert.equal(snapshot.rawText, rawText)
  assert.equal(parseSnapshot('module.exports={\"datasetAsOf\":\"2025-12\",\"datasetVersion\":\"legacy\"}\n').datasetAsOf, '2025-12')
  assert.throws(() => parseSnapshot('const ignored = true\nmodule.exports={\"datasetAsOf\":\"2026-06\"}\n'), /approved JSON export/)
  assert.throws(() => parseSnapshot("module.exports=require('../utils/other-codec.js').decodeSnapshot({})\n"), /approved JSON export/)
  assert.throws(() => parseSnapshot("module.exports=require('../utils/snapshot-codec.js').decodeSnapshot({\"datasetAsOf\":})\n"), /invalid JSON/)
})
