...
const historicalCorrectionReplay = await readFile(resolve(root, '.github/workflows/historical-correction-replay.yml'), 'utf8')
...
test('status deployment is separately protected, and its limited rehearsal keeps release authorization closed', () => {
  assert.match(statusDeployment, /workflow_run:\s+workflows: \[monthly-data-check\]/)
  assert.match(statusDeployment, /workflow_dispatch:\s+inputs:\s+source_run_id:/)
  ...
  assert.match(statusDeployment, /github\.ref != format\('refs\/heads\/\{0\}', github\.event\.repository\.default_branch\)/)
  ...
  assert.match(statusDeployment, /run\.path === '\.github\/workflows\/monthly-data-check\.yml'/)
  assert.match(statusDeployment, /source run is not bound to the current default-branch commit/)
  assert.match(statusDeployment, /node --input-type=module <<'NODE'/)
  assert.match(statusDeployment, /import \{ appendFileSync \} from 'node:fs'/)
  assert.doesNotMatch(statusDeployment, /const fs = require\('fs'\)/)
  assert.match(statusDeployment, /status-deployment-rehearsal/)
  ...
  assert.match(statusDeployment, /vars\.AUTOMATIC_RELEASE_ENABLED == 'false'/)
  assert.match(statusDeployment, /CI_STATUS_DEPLOYMENT_MODE:/)
  assert.match(statusDeployment, /CI_STATUS_DEPLOYMENT_REHEARSAL_CONFIRMATION:/)
  ...
  assert.match(statusDeploymentScript, /status deployment rehearsal requires the repository automatic release authorization to remain false/)
  assert.match(statusDeploymentScript, /status deployment rehearsal requires the production environment authorization to remain false/)
  assert.match(statusDeploymentScript, /status deployment workflow reference is invalid/)
...
test('independent watchdog can only read Actions runs and dispatch the fixed discovery workflow', () => {
  ...
  assert.match(watchdogFunction, /WATCHDOG_DRY_RUN/)
  assert.match(watchdogFunction, /cloud\.init\(\{ env: cloud\.DYNAMIC_CURRENT_ENV \}\)/)
  assert.match(watchdogFunction, /database\.runTransaction/)
  assert.match(watchdogFunction, /already_claimed/)
...
test('publisher is triggered only by the named discovery workflow', () => {
  ...
  assert.match(publisher, /Wait for the exact base commit's CI/)
  assert.match(publisher, /for attempt in \$\(seq 1 60\)/)
  assert.match(publisher, /sleep 10/)
  assert.match(publisher, /\["push", "workflow_dispatch"\]\.includes\(run\.event\)/)
  assert.match(publisher, /run\.head_sha === process\.env\.BASE_SHA/)
  assert.match(publisher, /BASE_CI_RUNS="\$runs_json" BASE_SHA="\$BASE_SHA" node --input-type=module -e/)
...
test('queued duplicate discoveries recheck durable state before candidate work', () => {
  ...
  const candidateWork = prepare.indexOf('Fetch, audit, build, and test candidate')
  assert.ok(recheck >= 0 && candidateWork > recheck)
  assert.match(prepare, /isSameReleaseHandled/)
  assert.match(prepare, /readRemoteJson\('data\/releases\/auto-update-state\.json'\)/)
  assert.match(prepare, /readRemoteJson\('data\/releases\/pending-auto-release\.json'\)/)
  assert.match(prepare, /steps\.recheck_state\.outputs\.handled != 'true'/)
})

test('automatic publisher exposes only the isolated candidate directory', () => {
  const prepare = publisher.slice(publisher.indexOf('  prepare:'), publisher.indexOf('  publish:'))
...
test('scheduled recovery restores the original candidate artifact instead of generating a new one', async () => {
  ...
  assert.match(recoveryGate, /work\/auto-release\/candidate/)
  assert.doesNotMatch(recoveryGate, /work\/miniprogram-data/)
})
...
test('full historical replay can write only below its numeric rehearsal prefix', async () => {
  ...
  assert.match(fullReplay, /default:\s*"6"/)
  assert.match(fullReplay, /options:\s*\n\s*- "6"(?:\s|$)/)
  assert.doesNotMatch(fullReplay, /-\s*"(?:12|36)"/)
  assert.match(fullReplay, /npm run data:audit -- --report-only/)
  assert.match(fullReplay, /AUTO_RELEASE_AUDIT_REPORT_PATH: work\/full-auto-update-replay\/audit-report-\$\{\{ github\.run_id \}\}-\$\{\{ github\.run_attempt \}\}\.json/)
  ...
  assert.match(script, /argument\("months"\) \?\? "6"/)
...
test('historical correction replay is manual, isolated, and cannot affect production authorization', async () => {
  const script = await readFile(resolve(root, 'scripts/miniprogram/replay-historical-corrections-cloud.mjs'), 'utf8')
  assert.match(historicalCorrectionReplay, /workflow_dispatch:/)
  assert.match(historicalCorrectionReplay, /environment: housing-data-production/)
  assert.match(historicalCorrectionReplay, /miniprogram:data:replay-historical-cloud/)
  assert.match(historicalCorrectionReplay, /--rounds=12/)
  assert.match(historicalCorrectionReplay, /npm run data:audit -- --report-only/)
  assert.match(historicalCorrectionReplay, /AUTO_RELEASE_AUDIT_REPORT_PATH: work\/historical-correction-replay\/audit-report-\$\{\{ github\.run_id \}\}-\$\{\{ github\.run_attempt \}\}\.json/)
  assert.match(historicalCorrectionReplay, /timeout-minutes: 45/)
  assert.doesNotMatch(historicalCorrectionReplay, /AUTOMATIC_RELEASE_ENABLED|PRODUCTION_RELEASE_AUTHORIZED|tcb login/)
  assert.match(script, /housing-data\/rehearsals\/\$\{runId\}\/historical-corrections\//)
  assert.match(script, /assertRehearsalKey\(/)
  assert.match(script, /AUTO_RELEASE_AUDIT_REPORT_PATH/)
  assert.doesNotMatch(script, /putObject\([`'"]housing-data\/(?:current\.json|releases\/)/)
  assert.match(script, /intentional isolated pre-switch interruption/)
  assert.match(script, /intentional isolated post-switch guard failure/)
  assert.match(script, /unsafe_rollback_rejected: true/)
  assert.match(script, /production_pointer_untouched: true/)
