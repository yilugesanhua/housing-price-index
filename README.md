...
当前实现基线为 `apps/web/` 和 `apps/miniprogram/`；小程序唯一机器可读源码版本来自 `apps/miniprogram/config/version.js`，截至2026-08-05的当前值为 `v2.5.6`。本行只是当前快照，不是独立版本源。微信开发者工具目录 `70城小程序技术验证/` 是同步副本，源码版本不代表微信平台已经审核或发布。文档入口及当前/历史状态见 [docs/DOCUMENT_INDEX.md](docs/DOCUMENT_INDEX.md)。
