...
5. 每天完整窗口复核必须取得全部27个时段的实际记录。严格发现的有限观察门槛则按`MONTHLY_DATA_AUTOMATION_PLAN.md`执行：连续10个计划发现时段均须单次、准时、成功且与唯一观察对象身份一致。完整日审计可作为该10次门槛的证据，但27次是每日审计完整性要求，不是额外的观察数量门槛。
