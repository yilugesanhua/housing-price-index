...
> **状态：历史候选版本证据（只读）**
> 本文仅记录 `v2.3.0` 当时的自动审查，不是当前发布门禁，也不证明当前 `v2.4.0` 已完成开发者工具、双真机、微信审核或正式发布。
...
- 后续受保护发布运行 `30418908562` 已用修正版源数据生成新的不可变云端版本 `2026-06-e9788d0bddf3` 并完成原子切换；独立只读复核运行 `30419441865` 确认源版本 `2026-06-4fd1d1a8ff12`、70城完整重建、云函数、指针哈希和发布审计全部通过。两个旧远程版本保留审计但不可回滚，新指针不引用旧版本作为回滚目标。
