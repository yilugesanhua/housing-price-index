...
- 2026-08-28，维护人将严格发现的有限观察门槛从“连续3个完整发现窗口”改为“连续10个计划发现时段”。2026-08-27的27条连续单次、准时、成功且观察身份逐项一致的业务发现记录已覆盖新门槛，因此该观察数量已满足；严格发现仍仅为 `passed_limited`。本变更不减少每日27个时段的审计要求，也不改变迟到、失败、重复、重试遗留或身份不一致时失败关闭并重新计数的规则；自动更新启用清单未全部通过前，两个生产发布开关必须继续保持关闭。
