...
- 当前修复为：`inspect` 在固定到 Node 22 后执行锁定依赖安装，再下载报告并导入检查模块；工作流安全测试固定该顺序和不可变 Action SHA。修复推送后的 GitHub 实际运行、候选状态与生产开关回读必须分别留证，不能由本地测试推断。
- 该记录只增加一个真实的准时发现时段和一次 GitHub 对同一对象的实际读取证据；09:15 至 17:55 的完整 27 个时段仍未完成，严格发现验收仍为 `passed_limited`，两个生产发布开关继续保持关闭。
