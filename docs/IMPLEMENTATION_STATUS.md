...
本组治理“当前版本能否被旧证据代替、发布包能否复现、Web声明是否属于当前构建、运维时间是否唯一”问题。2026-07-29的原始事实基线是提交 `c1b52a06be9c53dc16af7ec97cf2c574c7735496` 和源码版本 `v2.4.0`；当前源码版本为 `v2.5.15`，精确提交 `b854f8e6911986b556e45b760f27108596c24845` 已生成40文件本地不可变候选。已完成来源身份与构件身份分离、针对性本地测试和完整仓库检查；维护人确认该版本已上传、审核通过并正式发布，但尚无同提交CI、候选绑定的开发者工具重新编译和双真机记录、平台构建号/时间/截图、正式线上数据回读或稳定归档。旧 `v2.4.10`、`v2.4.9` 和更早候选的 CI、云端回放、构件和真机记录仅属历史候选，不能替代当前源码。文档模板或批准目标不能替代代码、云端、开发者工具、真机和平台证据。

| ID | 用户决策 | 当前问题 | 已批准目标或保留决定 | 实现状态 | 验证状态 | 当前阻断 |
| --- | --- | --- | --- | --- | --- | --- |
| R01 | `approved` | 当前源码为 `v2.5.15`；精确提交已生成本地不可变候选，维护人确认该版本已上传、审核通过并正式发布，但尚无同提交CI、候选绑定的开发者工具/双真机记录、平台构建号/时间/截图或正式线上数据回读 | 使用现有不可变候选补齐上线审查、交接、Android和iPhone全量验收及平台回读；旧版证据只读保留 | `partial` | `passed_limited` | 阻断候选与线上版本的可复核绑定、稳定归档、远程数据和自动更新结论 |
...
| ID | 当前实现依据 | 仍需关闭证据 |
| --- | --- | --- |
| R01 | `apps/miniprogram/config/version.js`；精确提交 `b854f8e6911986b556e45b760f27108596c24845` 的 `v2.5.15` 40文件本地候选；维护人于2026-08-06确认该版本已于2026-08-05上传、审核通过并正式发布；所有 `v2.5.6` 及更早候选、模板和真机记录均为历史证据 | 完成同提交CI、候选绑定的开发者工具重新编译和Android/iPhone结果、平台构建号/时间/截图、正式线上数据回读及完整交接 |
