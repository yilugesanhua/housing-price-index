...
## 2026-08-31 `v2.5.29` 官方日程超时重试修复（本地验证）

- GitHub Actions `33334918561` 在 `main@55f4cc84d94eb2fb8d4115ac8ae1e822f825bd57` 读取两个国家统计局日程时均在30秒后抛出 Node `TimeoutError`。共享发现契约此前没有把该错误分类为可重试网络故障，因此该时段在第一次超时后失败关闭，未执行既有的三次有限重试。
- `apps/miniprogram/cloudfunctions/monthlyDataWatchdog/discovery-contract.js` 现将 `TimeoutError` 纳入原有网络故障重试分类；新增回归测试模拟前两次超时、第三次成功，仍最多尝试三次。双官方日程、URL白名单、严格时段和失败关闭规则均未放宽。云函数源码变化按版本规则将唯一源码版本升为 `v2.5.29`，不改任何小程序页面。
- 当前候选本地验证：`node --test scripts/miniprogram/watchdog-cloud.test.mjs` 为19项通过，`npm run test:data` 为71项通过，`npm run check` 通过，`npm run test:e2e` 为39项通过、1项项目既有跳过。源码涉及的云函数和版本文件已与微信开发者工具同步副本逐文件SHA-256一致。
- 该版本尚未部署云函数、生成不可变候选、完成当前提交CI、开发者工具、双真机、微信平台或正式远程数据回读；2026-08-30对 `v2.5.28` 的平台确认只保留为历史事实，不能移作 `v2.5.29` 的通过证据。两个生产开关保持 `false`，未写入正式数据或改变正式指针。

## 2026-08-31 V18 外部证据采集准备

- 本次仅提交一条文档事实记录，用于取得真实 Pull Request CI 身份并验证待发布恢复流程不会把 Pull Request 或旧提交的 CI 当作当前默认分支恢复授权。此记录不表示 V18 已关闭，也不修改自动发布代码、正式数据、生产指针或两个生产开关。
