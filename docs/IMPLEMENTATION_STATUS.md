...
- 2026-08-28，维护人将严格发现的有限观察门槛从“连续3个完整发现窗口”改为“连续10个计划发现时段”。2026-08-27的27条连续单次、准时、成功且观察身份逐项一致的业务发现记录已覆盖新门槛，因此该观察数量已满足；严格发现仍仅为 `passed_limited`。同日进一步明确：第10个合格时段结束后立即复核并登记，不等待当天27个时段结束；后续时段继续按日程运行，17:55后如另行进行27时段审计，只用于健康监测和异常证据。迟到、失败、重复、重试遗留或身份不一致仍须失败关闭并从新的连续时段重新计数；自动更新启用清单未全部通过前，两个生产发布开关必须继续保持关闭。
