...
当前 `scripts/release-readiness.mjs`、`release/attestations.json` 和 `release/evidence-index.json` 使用 `schema_version: 2` 声明与独立证据索引。门禁会把声明绑定到精确源码提交、生产 `dist/` 的完整文件清单、`dist/data/manifest.json`、正式域名、证据索引哈希和有效期；任一文件或字段变化都会失败关闭。仓库现存文件是未填写的模板，因此 `release:check` 当前必须失败，不能据此声明 Web 已发布或可发布。
...
R06已把Web发布声明升级为构建级证据。`release/attestations.json` 必须使用 `schema_version: 2`，并至少包含：
...
- `source_commit_sha` 必须等于本次构建检出的精确提交，门禁会拒绝有已跟踪或未跟踪源码改动的工作树；分支名、短SHA或工作树当前状态不能替代。
...
校验器和自动测试覆盖提交不一致、构建内容变化、数据替换、域名变化、证据哈希错误、过期和未来时间。它们只证明本地门禁；真实正式构建、外部验收证据索引和线上回读仍未完成，R06保持 `implemented/passed_limited`。
...
Schema 2会拒绝提交SHA、构建清单、数据版本、数据清单、正式域名或证据索引任一不匹配，以及任一声明过期或来自未来。
