...
1. 确认在GitHub账号 `yilugesanhua` 下创建私有仓库及仓库名称。建议名称：`housing-price-index`。
...
6. 上述证据全部通过后，明确确认允许把GitHub Environment变量 `AUTOMATIC_RELEASE_ENABLED` 从 `false` 改为 `true`。

除身份验证、真机操作和最终确认外，其余仓库初始化、工作流配置、环境变量录入、测试执行和结果核对由Codex完成。

## GitHub配置目标

- 私有仓库默认分支启用保护，禁止强制推送，要求 `ci / verify` 通过。
- 创建受保护Environment：`housing-data-production`。
- Environment变量：`AUTOMATIC_RELEASE_ENABLED=false`。
