...
- I09关闭证据必须证明：只读发现只生成不可变观察报告；独立受保护状态部署作业只能在基线一致时保持数据/撤销身份、递增控制代次并写后回读，冲突或失败时原指针字节不变。当前仓库已有该作业的脚本、受保护工作流和本地故障测试，但尚无受保护Environment实际运行、最小权限现场回读或真实云端失败演练；因此I09仍未关闭，不能把观察结果当作已部署的用户可见状态。
