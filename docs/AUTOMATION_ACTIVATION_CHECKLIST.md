...
> 当前候选版本号为 `v2.5.18`；历史 `v2.5.16` 和 `v2.5.17` 候选不作为当前候选或发布证据。

本文只记录必须在外部账号、真实云环境或真机中完成的启用事项。代码实现和常规验证不要求维护人员手工执行命令。

当前源码版本以唯一机器源 `apps/miniprogram/config/version.js` 为准，本次清单快照为 `v2.5.18`。本轮修复提交 `be351cca8a36c26898f4f46da372ef63205991c2` 的普通 CI 已成功；微信开发者工具重新编译、双真机、微信平台审核/发布和正式数据链路回读仍未提供当前提交的证据。该状态不等于正式远程数据或自动更新能力已经验收，执行启用自动更新或生产写入前仍必须在正式数据链路重新读取。下文涉及旧版本的其他实施记录按其日期和固定版本解释。
