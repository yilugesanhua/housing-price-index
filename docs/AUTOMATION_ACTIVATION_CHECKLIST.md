...
当前源码版本以唯一机器源 `apps/miniprogram/config/version.js` 为准，本次清单快照为 `v2.4.1`。现有证据只证明 `v2.3.0` 历史候选曾上传到微信开发版本区，以及旧提交上的普通月度远程更新候选代码和云端演练存在；没有当前版本体验版、双真机、微信审核或正式发布证据。实际线上版本和线上客户端能力均无法根据仓库现场核验，执行启用或发布操作前必须在微信平台重新读取。下文涉及 `v2.0.2`、`v2.3.0` 或 `v2.4.0` 的其他实施记录按其日期和固定版本解释。
...
- COS/SCF 官方 SDK 联调、完整 70 城隔离上传/回读及有限指针/回滚演练已有历史通过记录；生产开关必须保持关闭，直到R02、D01-D16、I01-I03、I09、I12、开发者工具检查、双真机和微信审核全部完成。
...
4. 将绑定当前精确候选身份的小程序版本正式发布，并在微信公众平台现场回读线上版本，确认版本号、候选身份和发布时间与交接记录一致。“已上传开发版”“已提交审核”或“审核通过”均不能单独满足本项。
...
`v2.4.1` 已有以下三份当前模板，必须直接填写并绑定同一精确候选，不得另建替代记录；三份文件目前仍是未完成模板，不能作为通过证据：

- [上线审查单](MINIPROGRAM_V2_4_1_LAUNCH_AUDIT.md)
- [双真机验收单](MINIPROGRAM_V2_4_1_DEVICE_TEST.md)
- [发布交接单](MINIPROGRAM_V2_4_1_RELEASE_HANDOFF.md)

`v2.4.0` 和更早版本的模板或记录只作历史参考，不得直接作为 `v2.4.1` 的通过证据。
