...
> 当前源码版本号为 `v2.5.28`；历史 `v2.5.16`、`v2.5.17`、`v2.5.18`、`v2.5.25`、`v2.5.26` 和 `v2.5.27` 候选不作为当前版本的候选或发布证据。

本文只记录必须在外部账号、真实云环境或真机中完成的启用事项。代码实现和常规验证不要求维护人员手工执行命令。

当前源码版本以唯一机器源 `apps/miniprogram/config/version.js` 为准，本次清单快照为 `v2.5.28`。2026-08-30，维护人确认该版本的 Android/iPhone 双真机验收、微信审核和正式发布已完成；该确认不替代当前版本的不可变候选、精确提交、平台回读或正式数据链路证据。D18季度历史复核和D19故障通知闭环均已完成一次真实线上受控演练，具体运行与证据见 `IMPLEMENTATION_STATUS.md`；它们不改变自动发布开关。CloudBase严格发现器的既有线上观察结论和小程序平台发布均不替代其余自动数据更新门槛，两个生产开关继续保持关闭。
