...
当前源码版本以唯一机器源 `apps/miniprogram/config/version.js` 为准，本次清单快照为 `v2.5.15`。维护人于2026-08-06确认该版本已于2026-08-05上传、审核通过并正式发布；仓库尚未保存平台构建号、审核/发布时间、截图、正式线上数据回读或其与本地候选ID的可复核绑定。该用户确认不等于正式远程数据或自动更新能力已经验收，执行启用自动更新或生产写入前仍必须在微信平台和正式数据链路重新读取。下文涉及 `v2.0.2`、`v2.3.0`、`v2.4.0` 或 `v2.4.1` 的其他实施记录按其日期和固定版本解释。
...
4. 对维护人确认已发布的当前版本，在微信公众平台现场回读线上版本，补齐版本号、候选身份、构建号和发布时间，并确认其与交接记录一致。用户确认、已上传开发版、已提交审核或审核通过均不能单独满足本项。
