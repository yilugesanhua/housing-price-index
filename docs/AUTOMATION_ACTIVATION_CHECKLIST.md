...
> 当前源码候选版本号为 `v2.5.25`；历史 `v2.5.16`、`v2.5.17` 和 `v2.5.18` 候选不作为当前候选或发布证据。

本文只记录必须在外部账号、真实云环境或真机中完成的启用事项。代码实现和常规验证不要求维护人员手工执行命令。

当前源码版本以唯一机器源 `apps/miniprogram/config/version.js` 为准，本次清单快照为 `v2.5.25`。本轮 `npm run check`、`npm run test:e2e`、数据校验和相关云函数测试已通过，源码已同步到微信开发者工具技术验证目录。CloudBase 严格发现器已部署并启用每分钟触发器；观察对象写入、写后回读、GitHub 只读消费门禁及其最小权限身份的现场读取边界均已完成验证。完整27个发现时段的线上观察、GitHub 对同一时段观察对象的实际消费记录、微信开发者工具重新编译、双真机、微信平台审核/发布和稳定归档仍未提供当前候选的证据。两个生产开关继续保持关闭。
