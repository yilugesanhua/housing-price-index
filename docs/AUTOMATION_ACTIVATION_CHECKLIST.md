...
> 当前源码版本号为 `v2.5.29`；历史 `v2.5.16`、`v2.5.17`、`v2.5.18`、`v2.5.25`、`v2.5.26`、`v2.5.27` 和 `v2.5.28` 候选不作为当前版本的候选或发布证据。

本文只记录必须在外部账号、真实云环境或真机中完成的启用事项。代码实现和常规验证不要求维护人员手工执行命令。

当前源码版本以唯一机器源 `apps/miniprogram/config/version.js` 为准，本次清单快照为 `v2.5.29`。该版本将国家统计局请求抛出的真实 `TimeoutError` 纳入既有有限重试，未放宽双官方日程校验、未改小程序页面；因此 `v2.5.28` 的 Android/iPhone、微信审核和正式发布确认只能作为历史平台事实，不能替代 `v2.5.29` 的不可变候选、精确提交、平台回读或正式数据链路证据。D18季度历史复核和D19故障通知闭环均已完成一次真实线上受控演练，具体运行与证据见 `IMPLEMENTATION_STATUS.md`；它们不改变自动发布开关。CloudBase严格发现器的既有线上观察结论和小程序平台发布均不替代其余自动数据更新门槛，两个生产开关继续保持关闭。
...
- `housing-data-production` Environment 已创建；写入与只读监测 Secrets 的名称已配置在环境中。2026-08-25早先手动运行 `monthly-data-check`（`32749279961`）曾因仓库级只读监测 Secrets 为空而失败；随后仓库级和 `housing-data-production` Environment 均已回读到 `TENCENTCLOUD_MONITOR_SECRET_ID`、`TENCENTCLOUD_MONITOR_SECRET_KEY` 两个名称，新的只读发现运行 `32823542147` 和 `32836001672` 均成功完成。最新检查发现官方 `2026-07` 已发布、当前生产仍为 `2026-06`，只生成交接报告，未自动上线。该结果只证明名称存在和只读发现流程成功，不证明密钥值、生产发布或完整门槛已经通过。当前仓库级 `AUTOMATIC_RELEASE_ENABLED=false`、生产 Environment 级 `PRODUCTION_RELEASE_AUTHORIZED=false`，两级门禁仍失败关闭；变量关闭不等于V16已完整验证，真实受保护Environment执行、一真一假组合和未来启用时的独立授权证据仍未完成。
- 提交 `4fe02a4a6f856d5d29f58f8bf23d55585ae79505` 已在真实测试云完成两条互不替代的隔离证据：普通月度回放 `32835008415` 为 `2026-01` 至 `2026-06` 的单组 `6/6`，历史修订回放 `32835011782` 为 `12/12`；两次均只写入 `housing-data/rehearsals/`，报告确认生产指针和正式发布目录未触碰，生产开关为 `false`。回放前后只读检查的生产 `current.json` SHA-256 均为 `d15b9ea0727f2e88b6aa936a3959396e8673ac32c60c873931ffac8934d0989c`。这些结果关闭当前 R02 的两条回放缺口，但不替代云函数部署、开发者工具、双真机、平台审核/发布和正式数据链路证据。
...
- 当前运行模式是 `automation_disabled`，不是有人监督或无人值守自动化。D18和D19已完成有限范围的线上受控演练，但R02、D01-D16及I01-I03、I09、I12等其余适用门槛尚未全部关闭，生产开关不得变更。
- D18的季度只读复核和D19的GitHub Issue生命周期已分别完成真实运行、隔离处置或确认/恢复关闭演练；未来运行仍须持续留证。它们只解除对应的有限演练缺口，不单独授权自动发布、稳定归档或正式数据写入。
...
- I09关闭证据必须证明：只读发现只生成不可变观察报告；独立受保护状态部署作业只能在基线一致时保持数据/撤销身份、递增控制代次并写后回读，冲突或失败时原指针字节不变。为解除“先开开关才能演练”的循环，当前仓库还提供唯一受保护状态演练入口：仅默认分支、当前成功的 `monthly-data-check` 运行、固定确认词 `status-deployment-rehearsal` 且仓库/Environment 两开关均为精确 `false` 时可运行；脚本再次核对这四项，只允许状态字段与控制代次改变，并把数据/来源/清单/撤销身份写入非敏感回执。该入口不是普通发布入口，也不得用于修改数据版本或打开开关。I09仍须取得受保护Environment实际成功、最小权限现场回读及一次陈旧基线真实失败演练后才可关闭，不能把观察结果当作已部署的用户可见状态。
...
当前计划在每天09:15至17:55每20分钟进行一次只读发现，18:00停止。计划间隔缩短并不能替代实际云端验证：GitHub Actions仍可能排队、延迟或丢失定时任务，且新时段尚未完成完整线上观察。因此仍无资格声明60分钟正式SLA已生效；该时效声明缺口不放宽数据正确性门禁，必须在工作流、守护器和实际窗口均验证后才能关闭。
