...
> 当前源码候选版本号为 `v2.5.27`；历史 `v2.5.16`、`v2.5.17`、`v2.5.18`、`v2.5.25` 和 `v2.5.26` 候选不作为当前候选或发布证据。

本文只记录必须在外部账号、真实云环境或真机中完成的启用事项。代码实现和常规验证不要求维护人员手工执行命令。

当前源码版本以唯一机器源 `apps/miniprogram/config/version.js` 为准，本次清单快照为 `v2.5.27`。本轮继续补齐历史修订清单的完整信任链：发布候选复用、状态部署、发布后监测、人工/自动回滚和预览验证器均须下载、校验字节数与SHA-256并传入同一份修订清单；回滚到历史修订包也不得绕过该校验。D18、D19的脚本和工作流定义仍仅处于本地候选。它们尚未在GitHub线上运行，D19还缺仓库变量 `HOUSING_DATA_INCIDENT_OWNER` 和真实故障演练，因此不能关闭D18或D19，更不能称为无人值守。CloudBase严格发现器的既有线上观察结论只证明只读发现，不替代当前候选的开发者工具、双真机、微信审核/发布、稳定归档、正式远程数据链路或其他适用门槛。两个生产开关继续保持关闭。
...
- D18的季度只读复核和D19的GitHub Issue生命周期目前只有本地实现与故障注入测试；本清单继续将两者视为未关闭项。季度工作流首次线上运行、历史页面变化后的人工隔离处置，以及设置负责人后的一次真实“打开、确认、超时、恢复关闭”演练，均须作为独立外部证据登记。
