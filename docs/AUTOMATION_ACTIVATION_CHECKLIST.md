...
当前源码版本以唯一机器源 `apps/miniprogram/config/version.js` 为准，本次清单快照为 `v2.5.16`。本轮修复提交 `be351cca8a36c26898f4f46da372ef63205991c2` 的普通 CI 已成功；微信开发者工具重新编译、双真机、微信平台审核/发布和正式数据链路回读仍未提供当前提交的证据。该状态不等于正式远程数据或自动更新能力已经验收，执行启用自动更新或生产写入前仍必须在正式数据链路重新读取。下文涉及旧版本的其他实施记录按其日期和固定版本解释。
...
当前上午10:30终查后的12:00补查、下午14:57至16:00窗口及17:00补查，仍无法保证从正式页面任意可访问时点起60分钟内发现；这不改变既定SLA起点，但意味着现有调度尚无资格声明60分钟正式SLA生效。该时效声明缺口不放宽数据正确性门禁；后续调整调度必须另行修改工作流、规范和测试并完成云端验证。
