...
当前 `v2.5.15` 已从精确提交生成本地不可变候选；微信公众平台“线上版本”截图直接确认版本号为`2.5.15`、发布时间为2026-08-06 09:16:40，维护人确认审核通过和正式线上自检已完成。仍未完成同提交CI、绑定该候选的正式双真机记录、构建号、审核通过时间、正式数据链路明细或稳定归档。本仓库后续不执行额外上传、审核、发布、部署或稳定归档，除非取得新的明确授权；旧版本证据不得用于关闭当前版本门禁。候选身份和待补证据见 [`MINIPROGRAM_V2_5_15_RELEASE_HANDOFF.md`](MINIPROGRAM_V2_5_15_RELEASE_HANDOFF.md)。
