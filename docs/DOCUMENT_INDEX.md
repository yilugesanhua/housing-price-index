...
本文件是项目文档的统一入口。当前产品与界面事实以 `apps/web/` 和 `apps/miniprogram/` 为准；小程序唯一机器可读源码版本来自 `apps/miniprogram/config/version.js`，截至本次整理其值为 `v2.5.29`。本文中的版本文字只是该次整理快照，不是第二版本源。`70城小程序技术验证/` 是同步副本，`release/miniprogram/` 是只读稳定归档。源码版本、历史测试或旧交接单均不等于微信平台已审核或发布。
...
| [`MONTHLY_DATA_AUTOMATION_PLAN.md`](MONTHLY_DATA_AUTOMATION_PLAN.md) | 月度自动更新的当前执行基线：严格20分钟发现定义、实施顺序、故障处理、变更规则和验收边界；不替代数据契约、生产启用清单或实施状态登记 |
| [`MINIPROGRAM_DATA_UPDATE.md`](MINIPROGRAM_DATA_UPDATE.md) | 小程序自动数据更新、完整70城缓存、回滚和权限规则；具体北京时间和Cron仍是唯一权威来源；其中带日期的实施记录仅为 `historical_evidence`，当前生产模式和开关状态必须同时以实施状态登记、启用清单和外部平台现场核验为准 |
...
当前 `v2.5.29` 是包含月度严格只读发现器、历史修订身份校验、历史页面隔离复核、故障通知闭环以及国家统计局请求超时有限重试的当前源码版本，本机源码目录和微信开发者工具目录的本次涉及文件均已同步。2026-08-30维护人对 Android/iPhone 双真机验收、微信平台审核和正式发布的确认对应 `v2.5.28`，只登记历史客户端和平台状态；当前工作区尚未保留可回读的 `v2.5.29` 不可变候选 ZIP、精确源码提交、平台构建号/时间或正式数据链路回读，不能据此开启正式数据自动发布。旧 `v2.5.28`、`v2.5.27`、`v2.5.26`、`v2.5.25` 和 `v2.5.15` 的候选与平台记录均为历史证据，不能替代当前版本的身份补证。

## 历史证据与记录

以下文件只证明其标注日期、提交、运行或候选版本。它们不得用于证明当前 `v2.5.29` 已通过验收，也不得反向要求当前页面恢复旧设计：
...
7. 月度发现的具体北京时间和Cron只在 `MINIPROGRAM_DATA_UPDATE.md` 维护；`MONTHLY_DATA_AUTOMATION_PLAN.md` 规定执行顺序、严格发现定义、故障处理和验收边界，不复制Cron。可执行工作流仍须由CI与该权威时间表做规范化相等校验。
