...
本文件是项目文档的统一入口。当前产品与界面事实以 `apps/web/` 和 `apps/miniprogram/` 为准；小程序唯一机器可读源码版本来自 `apps/miniprogram/config/version.js`，截至本次整理其值为 `v2.5.28`。本文中的版本文字只是该次整理快照，不是第二版本源。`70城小程序技术验证/` 是同步副本，`release/miniprogram/` 是只读稳定归档。源码版本、历史测试或旧交接单均不等于微信平台已审核或发布。
...
当前 `v2.5.28` 是包含月度严格只读发现器、历史修订身份校验、历史页面隔离复核和故障通知闭环的当前源码版本，本机源码目录和微信开发者工具目录均声明该版本。2026-08-30，维护人确认 Android/iPhone 双真机验收、微信平台审核和正式发布均已完成。该确认只登记客户端和平台状态；当前工作区尚未保留可回读的 `v2.5.28` 不可变候选 ZIP、精确源码提交、平台构建号/时间或正式数据链路回读，不能据此开启正式数据自动发布。旧 `v2.5.27`、`v2.5.26`、`v2.5.25` 和 `v2.5.15` 的候选与平台记录均为历史证据，不能替代当前版本的身份补证。

## 历史证据与记录

以下文件只证明其标注日期、提交、运行或候选版本。它们不得用于证明当前 `v2.5.28` 已通过验收，也不得反向要求当前页面恢复旧设计：
