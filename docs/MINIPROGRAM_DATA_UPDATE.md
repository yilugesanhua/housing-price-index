...
当前源码基线为 `apps/miniprogram/`，唯一机器版本源 `apps/miniprogram/config/version.js` 截至2026-08-03的当前值为 `v2.4.11`。现有平台证据只证明旧候选曾上传到微信开发版本区，并有旧提交上的普通月度远程更新代码、云端演练和自动化测试；没有当前版本体验版、双真机、微信审核或正式发布证据，因此实际线上版本和线上客户端能力无法根据仓库现场核验，执行生产操作前必须在微信平台重新读取。下文带日期、运行ID、远程版本或指针哈希的内容统一属于 `historical_evidence`，只证明当时观察，不是当前生产状态或当前客户端能力。

截至2026-08-03，当前本地数据工具身份为生产解析器 `official-html-v8-product-housing-only-strict-release-date`、独立审计 `full-record-audit-v6` 和审计报告 `schema_version: 2`。解析器缺少可信官方发布日期时失败关闭；审计报告绑定每批记录、全量记录和来源索引SHA-256，审计后的批次内容发生变化时旧报告不得复用。126批、70,560条记录已完成本地全量重解析和独立审计；该本地结果不等于云端部署、生产写入或正式发布。
...
3. **远程文件布局**：`v2.5.1` 只允许一个包含业务数据的完整远程数据文件；可额外使用一个不含业务数据的轻量控制/清单文件，且仅记录版本、文件大小、SHA-256和身份绑定。现有 `bootstrap.json` 和 `cities/<city_id>.json` 为当前/旧协议与兼容证据，不得被误写成已满足此目标；实施时必须设计有版本边界的迁移和旧客户端兼容策略。
...
现有 `MINIPROGRAM_12_MONTH_REPLAY_*.md` 和 [`MINIPROGRAM_V2_4_1_12_MONTH_REPLAY.md`](MINIPROGRAM_V2_4_1_12_MONTH_REPLAY.md) 只记录指定旧版本和旧提交的历史证据。当前 `v2.4.11` 的三组36个月本地结果见 [`MINIPROGRAM_V2_4_11_108_MONTH_REPLAY.md`](MINIPROGRAM_V2_4_11_108_MONTH_REPLAY.md)；它仍不能替代当前版本的精确提交、真实隔离云端、开发者工具和双真机报告。尚无同时满足普通月度、历史修订和当前候选外部证据链的精确候选，因此R02保持 `partial/passed_limited`，不得据此开启生产自动发布。

使用 `npm run miniprogram:data:replay -- --months=36`，按时间顺序对最近36个已核验目标月份做确定性回放，并从第1轮重新启动三组独立执行。每组目标月为 `2023-07` 至 `2026-06`，基线为 `2023-06`。正式数据覆盖仍从 `2016-01` 开始；固定120个月客户端窗口早于该起点的部分只能作为四字段均为 `null`、发布日期为空的显式测试填充，不得伪造官方数据或进入生产来源记录。
...
- 不得使用2016年1月覆盖起点以前的月份伪造生产数据。客户端窗口需要覆盖起点前月份时，只允许显式 `null` 测试填充，并逐轮登记、保持空发布日期和禁止进入生产来源记录。
