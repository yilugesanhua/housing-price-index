...
### 2026-08-30 当前本地候选（未部署）

唯一机器版本源 `apps/miniprogram/config/version.js` 当前为 `v2.5.27`。本轮没有修改小程序页面：历史修订的 `revision-manifest.json` 现须在发布候选复用、状态部署、发布后监测、人工/自动回滚和预览验证器中按固定发布目录下载，并同时校验字节长度和 SHA-256；回滚到历史修订包同样必须携带该清单。`v2.5.27` 已同步到微信开发者工具目录，版本、运行时、主验证云函数和预览验证云函数共5个本次相关文件已逐一SHA-256一致；尚未重新编译、真机调试、上传、审核或发布。没有部署云函数、写入正式数据、修改正式指针或改变两个生产开关。2026-08-30的 `v2.5.26`、2026-08-26的 `v2.5.25` 记录和2026-08-06的 `v2.5.15` 平台截图均属于历史证据，不能替代当前候选身份。

当前源码基线为 `apps/miniprogram/`，唯一机器版本源 `apps/miniprogram/config/version.js` 截至2026-08-26的当前值为 `v2.5.25`；本轮只读发现器修复了云数据库“不存在文档”和系统 `_id` 字段写入边界，不修改小程序页面。源码与微信开发者工具技术验证目录已逐文件同步，尚未取得本候选的开发者工具重新编译、双真机、平台审核/发布或稳定归档证据。2026-08-06的 `v2.5.15` 线上截图和确认仍属于历史证据，不能替代当前候选身份。下文带日期、运行ID、远程版本或指针哈希的内容统一属于 `historical_evidence`，只证明当时观察，不是当前生产状态或当前客户端能力。
...
2026-08-30的本地实现由 `scripts/miniprogram/incident-lifecycle.mjs` 和 `housing-data-incident-lifecycle.yml` 提供：同一故障指纹只复用一个开放Issue，指定负责人可确认，逾期只升级一次，恢复操作必须附带64位SHA-256证据后才关闭原Issue。该工作流只拥有GitHub Issue权限，不读取腾讯云、正式数据或生产发布开关。它依赖仓库变量 `HOUSING_DATA_INCIDENT_OWNER` 指定可被分配的GitHub负责人；该变量未配置或尚未完成真实运行前，打开、确认、超时和恢复均仅有本地模拟证据，D19保持未关闭。
...
2026-08-30的本地实现为 `scripts/data/historical-page-audit.ts` 和 `quarterly-historical-page-audit.yml`：只枚举当前标准化数据实际引用的官方来源批次，重新下载原页面并比较原始SHA-256。哈希相同只输出只读报告；哈希变化仅在 `work/quarterly-historical-page-audit/` 生成待人工复核任务，绝不改写标准化数据、远程包、正式 `current.json` 或生产开关。工作流尚未在线运行，且真实变化后的人工隔离处置尚无证据，D18保持未关闭。
