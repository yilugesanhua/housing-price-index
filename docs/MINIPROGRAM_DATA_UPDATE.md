...
3. **远程文件布局**：`v2.5.0` 只允许一个包含业务数据的完整远程数据文件；可额外使用一个不含业务数据的轻量控制/清单文件，且仅记录版本、文件大小、SHA-256和身份绑定。现有 `bootstrap.json` 和 `cities/<city_id>.json` 为当前/旧协议与兼容证据，不得被误写成已满足此目标；实施时必须设计有版本边界的迁移和旧客户端兼容策略。
...
7. **实施与平台验证清单**：180个月/100,800条记录全量审计、表内统计月份正反向测试、完整文件哈希与重建测试、下载中断/半包/缓存提交失败/孤儿清理/离线回退测试、当前会话无重开与状态保持测试均须通过；Android和iPhone真机、Wi-Fi/移动网络/弱网场景，以及开发者工具主包不高于1.45MB仍须取得实测证据。在这些平台验证完成前，状态只能是 `partial/passed_limited`，生产自动发布继续关闭。
