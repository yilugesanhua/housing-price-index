...
当前源码基线为 `apps/miniprogram/`，唯一机器版本源 `apps/miniprogram/config/version.js` 截至2026-08-06的当前值为 `v2.5.15`。维护人于2026-08-06确认该版本已于2026-08-05上传、审核通过并正式发布；仓库尚未取得平台构建号、审核/发布时间、截图、正式线上数据回读或其与本地候选身份的可复核绑定。旧提交上的普通月度远程更新代码、云端演练和自动化测试仍只构成有限证据，不能据此推断当前线上远程数据或自动更新能力；执行生产操作前必须在微信平台和正式数据链路重新读取。下文带日期、运行ID、远程版本或指针哈希的内容统一属于 `historical_evidence`，只证明当时观察，不是当前生产状态或当前客户端能力。
...
`v2.4.0` 起建立受审计历史修订协议，当前 `v2.5.15` 源码继续保留相关控制面、双重撤销、受控回滚和客户端失败关闭；[实施状态登记](IMPLEMENTATION_STATUS.md) 仍有 `partial/not_started/not_tested` 项，[自动更新启用清单](AUTOMATION_ACTIVATION_CHECKLIST.md) 也未满足，不能描述为完整安全闭环。维护人确认 `v2.5.15` 已发布，但仓库尚未取得该发布与精确候选身份的可复核绑定或正式数据链路回读；执行生产操作前必须在微信平台和正式数据链路核验。线上客户端被证明支持当前协议前，同月历史修订仍必须同时发布新的云端不可变版本和包含相同修正版内置数据的小程序版本，不得把候选代码能力描述为线上客户端已经支持。
...
仓库内可复查的平台证据只覆盖旧 `v2.3.0` 开发版；维护人已确认 `v2.5.15` 上传、审核通过并正式发布，但尚未提供平台字段、截图或候选绑定。实际线上版本及普通月度能力仍必须在微信平台和正式数据链路现场核验。[自动更新启用清单](AUTOMATION_ACTIVATION_CHECKLIST.md) 未全部通过时，`AUTOMATIC_RELEASE_ENABLED` 不得开启；旧 `v2.4.1` 的普通CI运行 `30736720927` 不能替代当前候选的云端、开发者工具、双真机和正式数据链路证据，也不能证明线上客户端已经支持受审计历史修订协议。
