...
### 2026-08-31 当前源码与平台状态

唯一机器版本源 `apps/miniprogram/config/version.js` 当前为 `v2.5.29`。本机源码和微信开发者工具目录的 `config/version.js` 以及本次涉及的 `monthlyDataWatchdog/discovery-contract.js` 已逐文件核对一致；该版本只修复国家统计局请求的 `TimeoutError` 有限重试，不修改小程序页面、日程双源校验或失败关闭边界。2026-08-30维护人确认的 Android/iPhone 双真机验收、微信审核和正式发布对应 `v2.5.28`，只能作为历史平台事实，不能替代 `v2.5.29` 的不可变候选、精确提交、平台构建号/时间、主包大小、正式数据链路回读或稳定归档。尚未因为本次修复部署云函数、写入正式数据、修改正式指针或改变两个生产开关。

2026-08-26 的历史源码基线为 `v2.5.25`；当时只读发现器修复了云数据库“不存在文档”和系统 `_id` 字段写入边界，不修改小程序页面。该版本、`v2.5.28` 的平台确认及2026-08-06的 `v2.5.15` 线上截图和确认均属于历史证据，不能替代当前 `v2.5.29` 的身份补证。下文带日期、运行ID、远程版本或指针哈希的内容统一属于 `historical_evidence`，只证明当时观察，不是当前生产状态或当前客户端能力。

2026-08-26 当前月度自动更新执行：

- 腾讯云 `monthlyDataWatchdog` 已按 `MONTHLY_DATA_AUTOMATION_PLAN.md` 部署为严格时段只读发现器；当前唯一触发器为每分钟 `0 * * * * * *`，数据库集合保存固定 `slot_id`、租约、重试和观察状态。
- GitHub 继续只负责日程同步、独立审计、候选准备和未来受保护发布；腾讯云发现器没有正式数据写权限。`AUTOMATIC_RELEASE_ENABLED=false` 与 `PRODUCTION_RELEASE_AUTHORIZED=false` 均已现场回读。
- 当前正式指针只读回读为 `2026-06`，原始 `current.json` SHA-256 为 `d15b9ea0727f2e88b6aa936a3959396e8673ac32c60c873931ffac8934d0989c`；本轮没有生产数据、正式指针或小程序页面写入。
- 部署后的手动调用发生在18:00截止后，成功写入28条过期时段记录；这证明状态机可运行，不证明27个时段按时完成。下一步必须保留完整官方发布时间窗口的实际记录。
...
- 最小权限凭据和完整隔离云演练已完成；`v2.5.28` 的双真机、微信审核和正式发布已由维护人确认，但当前 `v2.5.29` 候选身份、平台验收和正式数据回读仍需补证。
...
后续月度自动更新按 [月度数据自动更新执行方案](MONTHLY_DATA_AUTOMATION_PLAN.md) 分工执行。腾讯云和 GitHub 不是两套可以互相替代的生产更新器，而是同一条流程中的不同职责；两边都使用 `monthlyDataWatchdog/discovery-contract.js` 的同一套日程、官方来源白名单、月份比较和交接身份规则。

- 腾讯云 `monthlyDataWatchdog` 是严格时段的唯一控制器：按北京时间计算时段，在 CloudBase 数据库用事务、可过期租约和固定 `slot_id` 领取任务，并直接完成只读发现和不可变观察报告。
- 腾讯云函数仅可读取固定的生产 `current.json`、访问国家统计局公开页面，以及写自己的时段和观察报告。它不能上传数据、修改 `current.json`、切换正式指针、修改小程序页面或使用生产数据写凭据。
- GitHub Actions 保留为独立审计、可复现报告、人工重放和候选构建入口。它的定时运行可以晚到或漏投，因此不是严格时段的时间权威；来自看门狗的补发必须原样带入同一个 `watchdog_slot`，不得重算或覆盖腾讯云已记录的 `late`、`failed` 或 `expired` 状态。
- 新月份只能先形成固定身份的发现交接，再由 GitHub 的独立候选、数据门禁和受保护发布流程处理。任何候选异常、权限异常或数据异常都保留上一份正式数据。
- 腾讯云成功观察会额外写入 `housing-data/discovery/observations/<sha256(slot_id)>.json` 非业务对象；GitHub 发现任务按同一 `slot_id` 只读回读并校验 `observation_id`、`payload_sha256`、`timing_status`、指针月份、官方URL、幂等键和 `handoff_identity`。该对象缺失、迟到或身份不一致时，候选和待发布恢复均失败关闭。
- 任何时候都只能有一个正式数据写入入口。当前只有受保护 GitHub 发布工作流在未来满足全部门槛时才可能获得该资格；腾讯云严格发现器永远没有正式数据写权限。

当前运行模式仍是 `automation_disabled`。`AUTOMATIC_RELEASE_ENABLED=false` 与 `PRODUCTION_RELEASE_AUTHORIZED=false` 或未设置时，发现、候选、补发和监测都不得写入正式数据。本文所称“自动上线”只指通过全部独立门槛后发布公开统计数据文件并原子切换 `current.json`；它不允许自动改变小程序页面、统计口径、登录、广告或支付，也不替代小程序代码审核。

本规范服从 [数据契约](DATA_CONTRACT.md)。外部账号、隔离验证、真机和正式启用事项只以 [自动更新启用清单](AUTOMATION_ACTIVATION_CHECKLIST.md) 为准。
...
- 自动发布工作流还必须接受同一 `slot_id` 的腾讯云观察对象；GitHub 自己的 `handoff.json`、运行 ID 或重新抓取结果都不能单独替代腾讯云严格时段证据。
...
| 腾讯云严格时段控制器 | 每分钟唤醒；只在09:15至17:55的20分钟时段执行发现 | `0 * * * * * *`（CloudBase 七段Cron） | 函数内部按北京时间领取27个唯一发现时段；18:00是截止边界，不额外开始新发现 |
| 每日定时发现 | 每天09:15至17:55，每20分钟 | `15,35,55 1-9 * * *` | GitHub 审计和备用信号；每次运行使用同一官方规则，但不作为严格时段的时间权威 |

`.github/workflows/monthly-data-check.yml` 与 `cloudbaserc.json` 是上表的可执行配置，必须与本节逐项一致。任何运行时刻变更都必须在同一提交中更新工作流、云函数配置与本节；`workflow-security.test.mjs` 会在 `npm run check` 中校验 GitHub 的完整Cron集合、规范中的时段以及守护器的同一时间窗口，避免三处漂移。

计划时刻在09:15至17:55之间的最大间隔为20分钟，覆盖范围不再局限于旧的上午、下午窗口。腾讯云控制器必须在计划时段后2分钟内真正开始只读发现；超过2分钟即使后来成功也记录为 `late`。GitHub Actions可能排队、延迟或丢失定时任务，因此只能作为审计与备用信号，不能替代该时段记录。SLA起点仍严格保持“正式数据页面可访问”，不得改用预告时间或其他替代起点掩盖缺口；在云端配置完成并通过完整窗口验证前，不得声称60分钟正式SLA已经生效。该项属于I08的已披露未关闭边界。

- GitHub每天北京时间09:00同步官方发布日程；每天09:15起至17:55止，每20分钟执行一次同规则的审计发现，18:00停止。脚本必须根据双源核验后的具体发布日期决定是否访问正式发布列表；非发布日期只更新并核验日程，不得把预告当成正式数据。
- 腾讯云控制器每分钟唤醒后只领取当前或遗漏的合法时段。重复事件、并发调用或已完成时段不得重复访问官方页面；遗漏时段应写入 `late`、`failed` 或 `expired` 的可审计结果，不能因后来 GitHub 成功而被静默抹去。
- GitHub 看门狗审计默认关闭（`WATCHDOG_GITHUB_AUDIT_ENABLED=false`）。即使未来单独开启，它也只能以原始 `watchdog_slot` 补发固定的 `monthly-data-check.yml`，不能发布数据、改写时段记录或扩大任何生产权限。
...
独立状态部署作业必须重新下载当前 `current.json` 和撤销登记，确认观察报告绑定的基线仍有效，保持 `dataset_version`、`source_dataset_version`、manifest身份和撤销身份不变，只更新允许的状态/检查字段并递增 `control_generation`，最后逐字节回读。监测作业继续只读。当前已具备脚本、工作流及“基线冲突不写入、存储写失败不改指针、写后回读”本地测试；受保护Environment实际运行、权限最小化现场回读和真实失败演练尚未完成，因此当前模式仍为 `automation_disabled`。维护人在计划发布日可以按未来 `supervised_automation` 的责任流程核对观察报告、发布、守卫和监测，但不能把报告生成等同于状态已部署，也不能因此开启生产开关。
...
D19已在2026-08-30完成一次真实受控演练：失败回调自动打开 Issue #11，随后 Actions `33300990998` 和 `33300991022` 完成负责人确认和带 SHA-256 恢复关闭。该闭环不改变当前 `automation_disabled` 状态；只有启用清单全部适用硬门槛关闭、两个生产开关均经维护人另行授权启用后，才可讨论有人监督或无人值守自动化。

通知实现由 `scripts/miniprogram/incident-lifecycle.mjs` 和 `housing-data-incident-lifecycle.yml` 提供：同一故障指纹只复用一个开放Issue，指定负责人可确认，逾期只升级一次，恢复操作必须附带64位SHA-256证据后才关闭原Issue。该工作流只拥有GitHub Issue权限，不读取腾讯云、正式数据或生产发布开关。仓库变量 `HOUSING_DATA_INCIDENT_OWNER=yilugesanhua` 已配置；未来每次真实运行仍须保留独立的打开、确认、升级和恢复证据。

发布后24小时继续监测云函数错误、下载失败和流量预算。无法获得客户端匿名聚合指标时，不得虚构“用户端全部成功”；以云端调用和下载指标作为有限证据。

### 6. 人工介入例外

D19受控演练已完成；在自动发布仍关闭期间，正常月份继续按启用清单执行人工巡检。以下任一情况进入人工隔离队列：
...
3. D17-D20分别限制跨年SLA、历史页面修订发现、通知闭环声明和共享计算逻辑重构。D18/D19已有有限范围线上证据，D17和D20及其他自动发布门槛仍按实施状态登记继续补证。
4. 微信开发者工具、双真机、`unavailable`状态、历史修订专门回放和当前版本上线证据仍须按对应编号补齐。普通月度更新的6轮回放只证明其指定提交、运行和有限场景。
...
`scripts/data/historical-page-audit.ts` 和 `quarterly-historical-page-audit.yml` 只枚举当前标准化数据实际引用的官方来源批次，重新下载原页面并比较原始SHA-256。哈希相同只输出只读报告；哈希变化仅在 `work/quarterly-historical-page-audit/` 生成待人工复核任务，绝不改写标准化数据、远程包、正式 `current.json` 或生产开关。2026-08-30线上运行 `33300833376` 检出171条页面哈希变化、0条失败；维护人确认后完成171/171数据级复核，业务字段变化为0，没有修订申请或生产写入。该结果证明一次真实隔离闭环，未来季度运行仍须持续留证。
...
### 当前普通月度隔离验证标准（2026-08-25 起永久生效）

自 2026-08-25 起，普通月度自动更新的隔离验证统一为**单组连续 6 个月**。每次涉及自动更新客户端、远程协议、解析器、审计器或发布门禁的当前候选，都必须从第 1 轮开始完成这 6 个月，并覆盖本节列出的来源解析、逐值核对、失败关闭、隔离上传/回读、指针保护和客户端激活检查。旧的 36 个月报告继续作为历史证据保存，不再是当前候选的月数门槛。

回放脚本仍可用 `--months=1..36` 做故障定位或诊断；非 6 个月的手工运行不构成当前 R02 或生产自动发布的启用证据。该标准只有在新的、带日期的明确决策同时更新本规范、启用清单、工作流和安全测试后才可改变；在此之前持续有效。

### 当前版本的两条独立证据链

每个改变自动更新客户端、远程协议、解析器、审计器或发布门禁的小程序版本，都必须在该版本的精确提交上重新形成两条互不替代的证据链：

1. **普通月度更新连续6个月单组执行**：按时间顺序完成最近6个已核验目标月份，共6轮。每次回放仍须覆盖官方来源解析、560条新增记录逐值核对、历史零变化、候选构建、损坏包拒绝、隔离上传与回读、隔离指针切换、完整70城客户端激活和切城零下载。旧版本较长或较短的报告只保留为历史/诊断证据，不能替代当前版本的单组6个月结果。
...
- 6轮严格顺序执行。任一轮失败立即停止，写入失败月份、已完成轮次、错误和问题清单；修复后必须从第1轮重新开始，后续月份不得掩盖前序失败。
