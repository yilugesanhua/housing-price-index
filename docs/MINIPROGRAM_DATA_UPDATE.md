...
### 2026-08-30 当前源码与平台状态

唯一机器版本源 `apps/miniprogram/config/version.js` 当前为 `v2.5.28`。本机源码和微信开发者工具目录的 `config/version.js` SHA-256 一致；2026-08-30维护人确认该版本已完成 Android/iPhone 双真机验收、微信审核和正式发布。该确认不替代当前版本的不可变候选、精确提交、平台构建号/时间、主包大小、正式数据链路回读或稳定归档。没有因为这次平台操作部署云函数、写入正式数据、修改正式指针或改变两个生产开关。`v2.5.27` 及更早版本记录均属于历史证据，不能替代当前版本的身份补证。

2026-08-26 的历史源码基线为 `v2.5.25`；当时只读发现器修复了云数据库“不存在文档”和系统 `_id` 字段写入边界，不修改小程序页面。该版本及2026-08-06的 `v2.5.15` 线上截图和确认均属于历史证据，不能替代当前 `v2.5.28` 的身份补证。下文带日期、运行ID、远程版本或指针哈希的内容统一属于 `historical_evidence`，只证明当时观察，不是当前生产状态或当前客户端能力。
...
- 最小权限凭据和完整隔离云演练已完成；`v2.5.28` 的双真机、微信审核和正式发布已由维护人确认，但当前候选身份和正式数据回读仍需补证。
