...
本文定义“住房小二”微信小程序的每月无人值守数据更新机制。当前状态为**`v2.2.0` 候选客户端、云端资源、首个远程数据版本和自动化流水线代码均已实现；客户端已通过微信开发者工具验证，自动化已通过本地全量检查、静态工作流检查和离线故障注入，但尚未在私有GitHub仓库、受保护生产环境和测试云环境完成联调**。当前微信已发布的 `v2.0.2` 不包含客户端更新层，仍只使用内置数据。首个包含完整自动更新能力的版本完成体验版、双真机和微信审核发布后，才可标记为对用户已启用。
...
- `automatic_release_enabled` 仍为关闭状态；私有仓库、分支保护、GitHub Environment、最小权限凭据、测试云演练、双真机和微信审核仍未完成。
...
| CI自动抓取、全量门禁、受限自动发布和私有审计 | 代码已实现并通过本地测试；待私有仓库与测试云联调 |
