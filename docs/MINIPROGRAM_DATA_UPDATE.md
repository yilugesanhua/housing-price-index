...
本文定义“住房小二”微信小程序的每月无人值守数据更新机制。当前状态为**`v2.2.0` 候选客户端、云端资源、首个远程数据版本和自动化流水线代码均已实现；私有GitHub仓库和生产Environment已创建，首轮云端CI已通过，但私有仓库分支保护、最小权限生产凭据、测试云演练和双真机验收尚未完成**。当前微信已发布的 `v2.0.2` 不包含客户端更新层，仍只使用内置数据。首个包含完整自动更新能力的版本完成体验版、双真机和微信审核发布后，才可标记为对用户已启用。
...
- 私有仓库 `yilugesanhua/housing-price-index`、`main` 分支和 `housing-data-production` Environment 已创建；初始提交 `4e0713e` 的 `ci / verify` 已通过。
- 仓库级和Environment级 `AUTOMATIC_RELEASE_ENABLED` 均为 `false`，生产Secrets为空，未修改线上数据。
- 当前GitHub套餐不支持私有仓库分支保护；必须升级GitHub Pro后再启用，项目不得为绕过限制改成公开仓库。
