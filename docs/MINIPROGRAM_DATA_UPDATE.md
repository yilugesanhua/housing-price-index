...
当前 `v2.3.0` 只允许“远端月份更新”或“同月且 `source_dataset_version` 与内置版本一致”的数据包；同月不同源版本继续失败关闭。下述“受审计历史修订协议”属于待实现能力，在支持该协议的小程序版本发布前，同月历史修订必须同时发布新的云端不可变版本和包含相同修正版内置数据的小程序版本。不得提前把规划中的接收规则描述为当前客户端已经支持。
...
受审计修订协议实现后，远程 `manifest.json` 必须新增并校验：
...
1. 定义并测试修订申请、逐记录差异和 `revision_manifest` schema。
2. 增加独立历史修订门禁和受保护发布工作流，继续复用现有全量解析、V4审计、70城整包、原子指针和只读监测能力。
3. 在小程序数据运行时实现修订链、撤销列表、完整包原子缓存和“无正确版本时不可用”状态。
4. 完成上述故障测试和12轮历史回放，随后发布首个支持受审计修订协议的小程序版本。
5. 该版本真实设备验证并上线后，后续一般历史修订才允许在人工批准后仅通过云端发布；重大修订仍按内置数据策略评估紧急小程序发版。
