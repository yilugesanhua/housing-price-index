...
| 下午发布前预检查 | 发布窗口内14:57 | `57 6 10-22 * *` | 覆盖国家统计局改为15:00发布的月份；非已核验发布日期不访问正式发布列表 |
| 下午高频发现 | 发布窗口内15:02至15:57，每5分钟 | `2,7,12,17,22,27,32,37,42,47,52,57 7 10-22 * *` | 只在双源核验后的具体发布日期检查 |
| 下午高频终查 | 发布窗口内16:00 | `0 8 10-22 * *` | 下午发布窗口仍未发现时停止高频检查、保留上月数据并通知 |
| 延迟补查 | 发布窗口内12:00、17:00 | `0 4,9 10-22 * *` | 覆盖高频窗口外的官方偶发延迟，不恢复全天高频轮询 |

`.github/workflows/monthly-data-check.yml` 是可执行配置，必须与上表逐项一致。任何运行时刻变更都必须在同一提交中更新工作流与本节，并由CI解析两边的规范化时间集合进行相等校验；只检查某几个Cron字符串或依赖人工目视不构成关闭证据。该一致性校验尚未实现；本轮已同步更新工作流和针对性安全测试，R07仍保持 `partial/not_tested`。

当前时刻表不能保证所有正式页面可访问时点都在60分钟内被发现：若页面在10:30之后出现，下一次12:00补查可能已经超过正式SLA目标；12:00之后出现则可能要到14:57。SLA起点仍严格保持“正式数据页面可访问”，不得改用预告时间或其他替代起点掩盖该缺口。当前只能把10至25分钟作为09:30或15:00常规发布且高频发现正常运行时的预期；在调度或等价监控补齐覆盖并完成云端验证前，不得声称60分钟正式SLA已经生效。该项属于I08的已披露未关闭边界。

- GitHub每天北京时间09:00同步官方发布日程。每月10日至22日只是Cron可唤醒范围，脚本必须根据双源核验后的具体发布日期决定是否访问正式发布列表，非发布日期只同步日程。
- 已核验发布日期保留国家统计局通常09:30发布的检查窗口：北京时间09:27预检查，09:32至10:27每5分钟检查一次，10:30终查。若官方日程明确为15:00发布，则北京时间14:57预检查，15:02至15:57每5分钟检查一次，16:00终查。任一窗口终查仍未发现时停止该窗口的高频检查、保留上月数据并通知；当天12:00和17:00各补查一次，以覆盖高频窗口外的官方偶发延迟。
