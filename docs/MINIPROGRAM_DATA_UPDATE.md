...
- 当前线上远程版本仍为 `2026-06-ec36ff8fb2e5`，源数据版本仍为修复前的 `2026-06-679ea146d4e2`；本次合并不切换生产 `current.json`。GitHub仓库级 `AUTOMATIC_RELEASE_ENABLED` 保持 `false`。
...
- 修复后的生产只读复核运行`30378918530`通过：线上70城包自身完整、发布与指针修复审计链匹配，状态为`known_stale_source`，客户端继续使用内置修正版；正式指针未被修改。
...
时效按“官方正式页面可访问”开始计算。正常09:30发布时，09:32至10:27每5分钟检查，发现等待通常为0至5分钟，另计GitHub少量排队时间；完整CI检查、构建和端到端测试通常需要5至15分钟；本次12轮云端实测每轮数据门禁、发布包、全量回读、隔离切换和客户端激活为25.6至62.6秒，平均40.2秒。正常目标为正式页面出现后10至25分钟可用，服务目标为30分钟，GitHub异常排队时保守目标为45分钟。若10:30前未发现，当天仅在12:00和16:00补查，不承诺30分钟目标。新用户打开或已有用户重新进入前台后即可检查新指针；任一数据门禁失败时不承诺更新时间，继续保留上月数据，符合“宁可不更新，也不能错误更新”的原则。
