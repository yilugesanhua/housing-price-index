...
| 每日定时发现 | 每天09:15至17:55，每20分钟 | `15,35,55 1-9 * * *` | 每天唤醒；18:00是截止边界，不额外运行。只有双源核验后的具体发布日期才访问正式发布列表 |

`.github/workflows/monthly-data-check.yml` 是可执行配置，必须与上表逐项一致。任何运行时刻变更都必须在同一提交中更新工作流与本节；`workflow-security.test.mjs` 会在 `npm run check` 中校验工作流的完整Cron集合、规范中的时段以及守护器的同一时间窗口，避免三处漂移。

计划时刻在09:15至17:55之间的最大间隔为20分钟，覆盖范围不再局限于旧的上午、下午窗口；但GitHub Actions可能排队、延迟或丢失定时任务，且新时段尚未完成线上观察。SLA起点仍严格保持“正式数据页面可访问”，不得改用预告时间或其他替代起点掩盖缺口；在工作流与守护器的线上配置完成并通过实际窗口验证前，不得声称60分钟正式SLA已经生效。该项属于I08的已披露未关闭边界。

- GitHub每天北京时间09:00同步官方发布日程；每天09:15起至17:55止，每20分钟执行一次只读发现，18:00停止。脚本必须根据双源核验后的具体发布日期决定是否访问正式发布列表；非发布日期只更新并核验日程，不得把预告当成正式数据。
- 独立守护器使用同一UTC时段识别遗漏的定时任务。守护器保持只读/干跑时，只记录应补查事实；只有其独立开关经现场验证后才可能补发同一发现工作流，仍不具备数据发布权限。
