...
使用 `npm run miniprogram:data:replay`，以已核验的相邻两个月做一次确定性回放。当前固定样本为2026年5月基线和2026年6月目标月，覆盖发布日程、正式页面发现、官方HTML重解析、候选门禁、70城发布包、隔离云上传、隔离指针切换和小程序客户端启用。

- 云端模式只能写入 `housing-data/rehearsals/<github-run-id>/full-auto-update/`；脚本通过 `assertRehearsalKey` 拒绝生产 `current.json` 和 `releases/` 路径。
- 正向回放必须证明新增恰好560条、70城各8种口径完整、历史记录零修改、官方HTML哈希固定、重解析结果与核验归档逐条一致。
- 发布包必须上传并回读 `manifest.json`、`bootstrap.json` 和70个城市分片，逐文件核对SHA-256，并在全部回读通过后才切换隔离指针。
- 客户端回放必须从2026年5月内置快照启动，刷新后激活2026年6月远程快照，并验证一个启动必需城市和一个按需城市分片。
- 反向回放至少故意删除一条官方记录；门禁必须在上传和指针切换前失败，指针内容及SHA-256必须保持不变。
...
时效按“官方正式页面可访问”开始计算。发现任务在发布窗口每30分钟轮询；正常情况下发现等待为0至30分钟，抓取、校验、构建、云端全量回读和指针切换预计10至25分钟。正常目标为正式页面出现后15至45分钟可用，保守服务目标为60分钟。任一数据门禁失败时不承诺更新时间，继续保留上月数据，符合“宁可不更新，也不能错误更新”的原则。
