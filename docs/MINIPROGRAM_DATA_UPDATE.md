...
本文定义“住房小二”微信小程序的每月无人值守数据更新机制。当前状态为**`v2.2.0` 候选客户端、云端资源、首个远程数据版本和自动化流水线代码均已实现；公开GitHub仓库、默认分支规则集、生产Environment、最小权限写入/监测凭据和完整隔离云演练均已完成，剩余门槛是开发者工具现场检查、Android/iPhone双真机验收和微信审核**。当前微信已发布的 `v2.0.2` 不包含客户端更新层，仍只使用内置数据。首个包含完整自动更新能力的版本完成体验版、双真机和微信审核发布后，才可标记为对用户已启用。
...
- 发布包必须上传并回读 `manifest.json`、`bootstrap.json` 和70个城市分片，逐文件核对SHA-256，并在全部回读通过后才切换隔离指针。
- 客户端每轮必须从上一月快照启动，刷新后激活目标月远程快照，并验证一个启动必需城市和一个按需城市分片。
- 每轮反向回放故意删除一条官方记录；门禁必须在上传和指针切换前失败，隔离指针内容及SHA-256必须保持不变。
- COS对象操作每批最多10个，单次60秒超时，最多3次幂等重试。三次失败后停止当轮并保留上一月指针，不得继续下一轮。
...
时效按“官方正式页面可访问”开始计算。发现任务在发布窗口每30分钟轮询，因此发现等待为0至30分钟；完整CI检查、构建和端到端测试通常需要5至15分钟；本次12轮云端实测每轮数据门禁、发布包、全量回读、隔离切换和客户端激活为25.6至62.6秒，平均40.2秒。正常目标仍为正式页面出现后15至45分钟可用，保守服务目标为60分钟。新用户打开或已有用户重新进入前台后即可检查新指针；任一数据门禁失败时不承诺更新时间，继续保留上月数据，符合“宁可不更新，也不能错误更新”的原则。
