...
- 官方HTML批次进入生产前执行 `npm run data:audit`，逐批校验原文SHA-256、官方URL、标题月份、发布日期，并确认记录所在表为对应的新房/二手房总体表或面积分类表，再按 `source_record_locator` 将全部标准记录回查到原始表格单元格；重复旧页面的面积表按分类表结构和解析器表序映射校验。审计报告保存为 `data/audit-report.json`。发布门禁只接受当前 `full-record-audit-v3` 方法，旧审计报告必须重跑。
