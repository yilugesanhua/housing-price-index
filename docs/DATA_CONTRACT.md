...
- `v2.5.1` 远程协议的业务数据必须物理保存为一个完整数据文件。允许另有不含任何业务数据的小型控制/清单文件，只提供版本、文件大小、哈希和身份绑定；当前 `bootstrap.json` 加 `cities/` 的布局是旧协议事实或兼容证据，不得被描述为已实现的单文件15年协议。
...
当前生产解析器为 `official-html-v8-product-housing-only-strict-release-date`。官方页面必须提供可验证且与统计月份相符的发布日期；缺失、无效或无法可信关联时必须阻断批次，不得回退为统计月1日或其他推算日期。
...
`[小程序][partial]` `v2.5.1` 的180个月协议已在本地候选实现：完整远程数据文件在首屏成功渲染后由客户端自动检查和后台下载；一次下载失败、网络中断或存储不足只保留已验证的完整快照，并在后续启动时自动重试。下载完成不要求用户点击“近15年”或重开小程序，且不得改变当前可见页面的选择状态。隔离预览、开发者工具和真机证据仍是独立待完成项。
